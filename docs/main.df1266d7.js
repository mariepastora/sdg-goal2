...
var _appearance = require("./order/appearance");
...
(function () {
    ...
    Promise.all([d3.csv(require('./data/undernourishment_2015.csv')), d3.csv(require('./data/stunting.csv'))]).then(ready).catch(function (err) {
        return console.log('Failed on', err);
    });

    function ready(_ref) {
        var _ref2 = _slicedToArray(_ref, 2),
            undernourished = _ref2[0],
            stunting = _ref2[1];
        ...
        // squarechart
        var sizeScale = d3.scaleLinear().domain([0, 60]).range([30, 230]);
        ...
        squares_graphic.selectAll('.cube').data(undernourished).enter()
        ...
        .style('margin-top', 50 + 'px').style('position', 'relative').classed('cube', true)
        //.append('div')
        ...
        .append('div').style('width', function (d) {
            ...
            return sizeScale(+d.percentage) - sizeScale(10.8) + 'px';
        }).style('margin-right', 10 + 'px').classed('world-div', true)
        // appending text div
        .append('div').style('width', function (d) {
        ...
        svg = container.append("svg").attr("class", "line_svg");
        ...
        var xPositionScale = d3.scaleLinear().domain([2000, 2018]).range([0, width]);

        var yPositionScale = d3.scaleLinear().domain([0, 55]).range([height, 0]);

        // Define the line
        var line = d3.line().curve(d3.curveStep).x(function (d) {
            return xPositionScale(+d.TimePeriod);
        }).y(function (d) {
            return yPositionScale(+d.Value);
        });

        var tooltip = d3.select("body").append("div").style("position", "absolute").style("z-index", "10").style("opacity", "0").style("background", "#000").text("a simple tooltip").classed('tooltip', true);

        svg.selectAll(".line").data(nest).enter().append("path").classed("line", true).attr("d", function (d) {
            console.log('1', line(d.values));
            return line(d.values);
        }).attr('class', function (d) {
            return d.key.toLowerCase().replace(/\s/g, '');
        ...
        }).attr('stroke-width', '1.5px').on('mouseover', function (d) {
            console.log(d);
            ...
            tooltip.html(d.key).style("left", d3.event.pageX + 10 + "px").style("top", d3.event.pageY - 28 + "px").style('background-color', 'white').style('border', '1px solid black').style('font-size', '0.8em').style('padding', '3px').style('hyphens', 'auto');
        ...
        var xaxis = svg.append("g").attr("transform", "translate(0," + height + ")").attr("class", "x axis").call(d3.axisBottom(xPositionScale)
        //.ticks(d3.timeMonth)
        //.tickSize(12)
        .tickFormat(d3.format("d")).tickSizeInner(5).tickPadding(7));

        // Add the Y Axis
        var yaxis = svg.append("g").attr("class", "y axis").classed('y-axis', true).call(d3.axisRight(yPositionScale).ticks(5).tickSizeInner(5).tickPadding(6).tickSize(width).tickFormat(function (d) {
...
},{"d3":"../node_modules/d3/index.js","./data/undernourishment_2015.csv":"data/undernourishment_2015.csv","./data/stunting.csv":"data/stunting.csv"}],"../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/main.df1266d7.map